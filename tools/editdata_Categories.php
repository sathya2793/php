...
if(isset($_REQUEST['id'])){
    ...
    $sql="select * from public.categories WHERE id=$id";
    $run_sql=pg_query($sql);
    while($row=pg_fetch_assoc($run_sql)){
        $per_id=$row[0];
        $per_topic=$row[1];
        $per_title=$row[2];
                ...
                <form class="form-horizontal" method="post">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="text" class="form-control" id="txttopic" name="txttopic" value="<?php echo $per_name;?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-4 control-label" for="txttitle">Title</label>
                            <div class="col-sm-6">
                                <input type="number" class="form-control" id="txttitle" name="txttitle" value="<?php echo $per_title;?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-4 control-label" for="txtdescription">Description</label>
                            <div class="col-sm-6">
                                <input type="number" class="form-control" id="txtdescription" name="txtdescription" value="<?php echo $per_description;?>">
                            </div>
                        </div>
                    </div>
                </form>
